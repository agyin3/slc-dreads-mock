...
const AboutInfo = ({ scroll, height }) => {
    ...
    return (
        <InfoWrapper>
            <Slide direction='right' in={scroll >= height * 2.1} timeout={1000}>
            <Img className={styles.aboutImg} fluid={data.image1.childImageSharp.fluid} />
            </Slide>
            <TextWrapper>
                <Slide direction='left' in={scroll >= height * 2.1} timeout={1000}>
                <InfoHeading variant='h2' component='p' gutterBottom>
                    Making Utah Cool<br/> One DreadLock At A Time
                </InfoHeading>
                </Slide>
                
                <Slide direction='up' in={scroll >= height * 2.1} timeout={1000}>
